...
Thank you for helping us make Cherry better!

All activity in Cherry forums is subject to our [Code of
Conduct](https://kriaa.in/cherry/code-of-conduct). Additionally, contributors must sign
our [Contributor License Agreement](https://kriaa.in/cherry/cla) before their
contributions can be merged.

## Contribution ideas

Cherry is a large project with a number of priorities. We spend most of
...
The Cherry culture values working code and synchronous conversations over long
discussion threads.
...
- Anything that can be provided by an extension. For example a new language, or theme. For adding themes or support for a new language to Cherry, check out our [docs on developing extensions](https://kriaa.in/cherry/docs/extensions/developing-extensions).
- New file icons. Cherry's default icon theme consists of icons that are hand-designed to fit together in a cohesive manner, please don't submit PRs with off-the-shelf SVGs.
...
## Bird's-eye view of Cherry

We suggest you keep the [Cherry glossary](docs/src/development/glossary.md) at your side when starting out. It lists and explains some of the structures and terms you will see throughout the codebase.

Cherry is made up of several smaller crates - let's go over those you're most likely to interact with:

- [`gpui`](/crates/gpui) is a GPU-accelerated UI framework which provides all of the building blocks for Cherry. **We recommend familiarizing yourself with the root level GPUI documentation.**
- [`editor`](/crates/editor) contains the core `Editor` type that drives both the code editor and all various input fields within Cherry. It also handles a display layer for LSP features such as Inlay Hints or code completions.
- [`project`](/crates/project) manages files and navigation within the filetree. It is also Cherry's side of communication with LSP.
...
- [`ui`](/crates/ui) is a collection of UI components and common patterns used throughout Cherry.
- [`cli`](/crates/cli) is the CLI crate which invokes the Cherry binary.
- [`zed`](/crates/zed) is where all things come together, and the `main` entry point for Cherry.

## Packaging Cherry

Check our [notes for packaging Cherry](https://kriaa.in/cherry/docs/development/linux#notes-for-packaging-zed).
