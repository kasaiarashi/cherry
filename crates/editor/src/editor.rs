...
impl Editor {
            ...
            if let Some(url) = url {
                cx.update(|window, cx| {
                    if parse_zed_link(&url, cx).is_some() {
                        window.dispatch_action(Box::new(cherry_actions::OpenCherryUrl { url }), cx);
                    } else {
                        cx.open_url(&url);
                    }
                })?;
                ...
                match first_url_or_file {
                    Some(Either::Left(url)) => {
                        cx.update(|window, cx| {
                            if parse_zed_link(&url, cx).is_some() {
                                window
                                    .dispatch_action(Box::new(cherry_actions::OpenCherryUrl { url }), cx);
                            } else {
                                cx.open_url(&url);
                            }
                        })?;
    ...
    pub fn copy_path(
        &mut self,
        _: &cherry_actions::workspace::CopyPath,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn copy_relative_path(
        &mut self,
        _: &cherry_actions::workspace::CopyRelativePath,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("see-docs", "See Docs").size(ButtonSize::Compact).on_click(|_ev, _window, cx| {
                            cx.open_url("https://kriaa.in/cherry/docs/completions#edit-predictions-missing-keybinding");
                        })),
                )
        })
    }
