//! Paths to locations used by Cherry.
...
/// On macOS, this is `~/Library/Application Support/Cherry`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/cherry`.
/// On Windows, this is `%LOCALAPPDATA%\Cherry`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/cherry`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/cherry`.
/// On Windows, this is `%APPDATA%\Cherry`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the cherry_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cherry_server").unwrap());
    *CACHED
}
...
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cherry_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by Cherry.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("cherry")
        }
    })
}

/// Returns the path to the data directory used by Cherry.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Cherry")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
/// Returns the path to the temp directory used by Cherry.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("cherry")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Cherry")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Cherry server directory on this SSH host.
...
/// Returns the path to the `Cherry.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Cherry.log"))
}

/// Returns the path to the `Cherry.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Cherry.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Cherry repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Cherry.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Cherry.
...
/// Returns the relative path to a `.cherry` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cherry/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cherry/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cherry/debug.json").unwrap());
    *CACHED
}
