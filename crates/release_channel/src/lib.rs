//! Provides constructs for the Cherry app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("CHERRY_RELEASE_CHANNEL")
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Cherry-Editor-Dev",
        ReleaseChannel::Nightly => "Cherry-Editor-Nightly",
        ReleaseChannel::Preview => "Cherry-Editor-Preview",
        ReleaseChannel::Stable => "Cherry-Editor-Stable",
    }
}

/// The Git commit SHA that Cherry was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("CHERRY_APP_VERSION") {
            from_env.parse().expect("invalid CHERRY_APP_VERSION")
        } else {
...
/// A Cherry release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Cherry.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "Cherry Dev",
            ReleaseChannel::Nightly => "Cherry Nightly",
            ReleaseChannel::Preview => "Cherry Preview",
            ReleaseChannel::Stable => "Cherry",
        }
    }
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Cherry on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "kriaa.kasaiarashi.Cherry-Dev",
            ReleaseChannel::Nightly => "kriaa.kasaiarashi.Cherry-Nightly",
            ReleaseChannel::Preview => "kriaa.kasaiarashi.Cherry-Preview",
            ReleaseChannel::Stable => "kriaa.kasaiarashi.Cherry",
        }
    }
