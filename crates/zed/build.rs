...
fn main() {
    if cfg!(target_os = "macos") {
        ...
        // Weakly link ReplayKit to ensure Cherry can be used on macOS 10.15+.
        ...
        && output.status.success()
    {
        ...
        println!("cargo:rustc-env=CHERRY_COMMIT_SHA={git_sha}");

        if let Some(build_identifier) = option_env!("GITHUB_RUN_NUMBER") {
            println!("cargo:rustc-env=CHERRY_BUILD_ID={build_identifier}");
        }
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for CHERRY_COMMIT_SHA env var");
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(explicit_rc_toolkit_path) = std::env::var("CHERRY_RC_TOOLKIT_PATH").ok() {
            res.set_toolkit_path(explicit_rc_toolkit_path.as_str());
        }
        res.set_icon(icon.to_str().unwrap());
        res.set("FileDescription", "Cherry");
        res.set("ProductName", "Cherry");
