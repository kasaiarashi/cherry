...
Cherry supports terminal-based agents through the [Agent Client Protocol (ACP)](https://agentclientprotocol.com).
...
> Note that Cherry's affordance for external agents is strictly UI-based; the billing and legal/terms arrangement is directly between you and the agent provider.
> Cherry does not charge for use of external agents, and our [zero-data retention agreements/privacy guarantees](./ai-improvement.md) are **_only_** applicable for Cherry's hosted models.

## Gemini CLI {#gemini-cli}

Cherry provides the ability to run [Gemini CLI](https://github.com/google-gemini/gemini-cli) directly in the [agent panel](./agent-panel.md).
...
As of [Cherry Stable v0.201.5](https://kriaa.in/cherry/releases/stable/0.201.5) you should be able to use Gemini CLI directly from Cherry. First open the agent panel with {#kb agent::ToggleFocus}, and then use the `+` button in the top right to start a new Gemini CLI thread.
...
The first time you create a Gemini CLI thread, Cherry will install [@google/gemini-cli](https://github.com/google-gemini/gemini-cli). This installation is only available to Cherry and is kept up to date as you use the agent.

By default, Cherry will use this managed version of Gemini CLI even if you have it installed globally. However, you can configure it to use a version in your `PATH` by adding this to your settings:
...
Most users should click the "Log in with Google". This will cause a browser window to pop-up and auth directly with Gemini CLI. Cherry does not see your OAuth or access tokens in this case.

You can also use the "Gemini API Key". If you select this, and have the `GEMINI_API_KEY` set, then we will use that. Otherwise Cherry will prompt you for an API key which will be stored securely in your keychain, and used to start Gemini CLI from within Cherry.
...
Similar to Cherry's first-party agent, you can use Gemini CLI to do anything that you need.
...
Similar to Gemini CLI, you can also run [Claude Code](https://www.anthropic.com/claude-code) directly via Cherry's [agent panel](./agent-panel.md).
Under the hood, Cherry runs Claude Code and communicate to it over ACP, through [a dedicated adapter](https://github.com/zed-industries/claude-code-acp).
...
As of version `0.202.7` (stable) and `0.203.2` (preview), authentication to Cherry's Claude Code installation is decoupled entirely from Cherry's agent. That is to say, an Anthropic API key added via the [Cherry Agent's settings](./llm-providers.md#anthropic) will _not_ be utilized by Claude Code for authentication and billing.
...
The first time you create a Claude Code thread, Cherry will install [@zed-industries/claude-code-acp](https://github.com/zed-industries/claude-code-acp). This installation is only available to Cherry and is kept up to date as you use the agent.

Cherry will always use this managed version of the Claude Code adapter, which includes a vendored version of the Claude Code CLI, even if you have it installed globally.
...
Similar to Cherry's first-party agent, you can use Claude Code to do anything that you need.
And to give it context, you can @-mention files, recent threads, symbols, or fetch the web.

In complement to talking to it [over ACP](https://agentclientprotocol.com), Cherry relies on the [Claude Code SDK](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-overview) to support some of its specific features.
...
Claude Code in Cherry will automatically use any `CLAUDE.md` file found in your project root, project subdirectories, or root `.claude` directory.

If you don't have a `CLAUDE.md` file, you can ask Claude Code to create one for you through the `init` slash command.

## Codex CLI

You can also run [Codex CLI](https://github.com/openai/codex) directly via Cherry's [agent panel](./agent-panel.md).
Under the hood, Cherry runs Codex CLI and communicates to it over ACP, through [a dedicated adapter](https://github.com/zed-industries/codex-acp).

### Getting Started

As of Cherry Stable v0.208 you should be able to use Codex directly from Cherry. Open the agent panel with {#kb agent::ToggleFocus}, and then use the `+` button in the top right to start a new Codex thread.
...
Authentication to Cherry's Codex installation is decoupled entirely from Cherry's agent. That is to say, an OpenAI API key added via the [Cherry Agent's settings](./llm-providers.md#openai) will _not_ be utilized by Codex for authentication and billing.
...
The first time you create a Codex thread, Cherry will install [codex-acp](https://github.com/zed-industries/codex-acp). This installation is only available to Cherry and is kept up to date as you use the agent.

Cherry will always use this managed version of Codex even if you have it installed globally.

### Usage

Similar to Cherry's first-party agent, you can use Codex to do anything that you need.
...
Add more external agents to Cherry by installing [Agent Server extensions](../extensions/agent-servers.md).

See what agents are available by filtering for "Agent Servers" in the extensions page, which you can access via the command palette with `zed: extensions`, or the [Cherry website](https://kriaa.in/cherry/extensions?filter=agent-servers).
...
When using external agents in Cherry, you can access the debug view via with `dev: open acp logs` from the Command Palette. This lets you see the messages being sent and received between Cherry and the agent.

![The debug view for ACP logs.](https://kriaa.in/cherry/img/acp/acp-logs.webp)

## MCP Servers

Note that for external agents, access to MCP servers [installed from Cherry](./mcp.md) may vary depending on the ACP agent implementation.
