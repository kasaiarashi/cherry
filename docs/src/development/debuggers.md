...
> **DISCLAIMER**: This is not documentation for [configuring Cherry's debugger](../debugger.md).
> Rather, it is intended to provide information on how to use a debugger while developing Cherry itself to both Cherry employees and external contributors.

## Using Cherry's built-in debugger

While the Cherry project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug Cherry with, one for GDB and one for LLDB. Select the configuration you want and Cherry will build and launch the binary.
...
Therefore, in order to use a debugger to it's fullest extent when debugging a release build, you must compile a new Cherry binary, with full debug info.
...
## Running Cherry with a shell debugger GDB/LLDB

### Background

When installing rust through rustup, (the recommended way to do so when developing Cherry, see the documentation for getting started on your platform [here](../development.md))
...
After following the steps above for including full debug info when compiling Cherry,
You can either run `rust-gdb` or `rust-lldb` on the compiled Cherry binary after building it with `cargo build`, by running one of the following commands:
...
Alternatively, you can attach to a running instance of Cherry (such as an instance of Cherry started using `cargo run`) by running one of the following commands:
...
Where `<pid>` is the process ID of the Cherry instance you want to attach to.

To get the process ID of a running Cherry instance, you can use your systems process management tools such as `Task Manager` on windows or `Activity Monitor` on macOS.
...
Debuggers can be an excellent tool for debugging the cause of panics and crashes in all programs, including Cherry.
...
You can find additional information on debugging Cherry crashes [here](./debugging-crashes.md).
