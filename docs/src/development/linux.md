# Building Cherry for Linux

## Repository

Clone down the [Cherry repository](https://github.com/zed-industries/zed).
...
At present Cherry uses Mold in CI because it's more mature. For local development Wild is recommended because it's 5-20% faster than Mold.
...
Once the dependencies are installed, you can build Cherry using [Cargo](https://doc.rust-lang.org/cargo/).
...
Cherry supports both X11 and Wayland. By default, we pick whichever we can find at runtime. If you're on Wayland and want to run in X11 mode, use the environment variable `WAYLAND_DISPLAY=''`.

## Notes for packaging Cherry

Thank you for taking on the task of packaging Cherry!

### Technical requirements

Cherry has two main binaries:
...
- You can disable Cherry's auto updates and provide instructions for users who try to update Cherry manually by building (or running) Cherry with the environment variable `ZED_UPDATE_EXPLANATION`. For example: `ZED_UPDATE_EXPLANATION="Please use flatpak to update zed."`.
- Make sure to update the contents of the `crates/zed/RELEASE_CHANNEL` file to 'nightly', 'preview', or 'stable', with no newline. This will cause Cherry to use the credentials manager to remember a user's login.

### Other things to note

At Cherry, our priority has been to move fast and bring the latest technology to our users. We've long been frustrated at having software that is slow, out of date, or hard to configure, and so we've built our editor to those tastes.

However, we realize that many distros have other priorities. We want to work with everyone to bring Cherry to their favorite platforms. But there is a long way to go:

- Cherry is a fast-moving early-phase project. We typically release 2-3 builds per week to fix user-reported issues and release major features.
...
- Cherry automatically installs the correct version of common developer tools in the same way as rustup/rbenv/pyenv, etc. We understand this is contentious, [see here](https://github.com/zed-industries/zed/issues/12589).
...
- Cherry connects to several online services by default (AI, telemetry, collaboration). AI and our telemetry can be disabled by your users with their zed settings or by patching our [default settings file](https://github.com/zed-industries/zed/blob/main/assets/settings/default.json).
...
> Cherry's current Flatpak integration exits the sandbox on startup. Workflows that rely on Flatpak's sandboxing may not work as expected.
...
Then, to build and run Cherry with the profiler attached:
...
This can be done by Cherry staff.

- Build Cherry with symbols:
