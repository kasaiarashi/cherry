...
If you are finding that Cherry is continually rebuilding root crates, it may be because
you are pointing your development Cherry at the codebase itself.
...
build of Cherry. These environment variables are in turn passed to `cargo check`, which
invalidates the build cache of some of the crates we depend on.

You can easily avoid running the built binary on the checked-out Cherry codebase using `cargo run
~/path/to/other/project` to ensure that you don't hit this.

### Speeding up verification

If you are building Cherry a lot, you may find that macOS continually verifies new
