...
Language support in Cherry has several components:
...
Each language supported by Cherry must be defined in a subdirectory inside the `languages` directory of your extension.
...
- `first_line_pattern` is a regular expression, that in addition to `path_suffixes` (above) or `file_types` in settings can be used to match files which should use this language. For example Cherry uses this to identify Shell Scripts by matching the [shebangs lines](https://github.com/zed-industries/zed/blob/main/crates/languages/src/bash/config.toml) in the first line of a script.
- `debuggers` is an array of strings that are used to identify debuggers in the language. When launching a debugger's `New Process Modal`, Cherry will order available debuggers by the order of entries in this array.
...
Cherry uses the syntax tree produced by the [Tree-sitter](https://tree-sitter.github.io) query language to implement
several features:
...
features in Cherry, using [JSON syntax](https://www.json.org/json-en.html) as a guiding example.
...
Cherry uses these to highlight matching brackets: painting each bracket pair with a different color ("rainbow brackets") and highlighting the brackets if the cursor is inside the bracket pair.
...
The `textobjects.scm` file defines rules for navigating by text objects. This was added in Cherry v0.165 and is currently used only in Vim mode.
...
For languages with closures, these typically should not count as functions in Cherry. This is best-effort however, as languages like JavaScript do not syntactically differentiate syntactically between closures and top-level function declarations.
...
Cherry uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) to provide advanced language support.
...
You can customize the handling of the language server using several optional methods in the `Extension` trait. For example, you can control how completions are styled using the `label_for_completion` method. For a complete list of methods, see the [API docs for the Cherry extension API](https://docs.rs/zed_extension_api).

### Multi-Language Support

If your language server supports additional languages, you can use `language_ids` to map Cherry `languages` to the desired [LSP-specific `languageId`](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocumentItem) identifiers:
