---
description: Cherry is a text editor that supports lots of Git features
title: Cherry Editor Git integration documentation
---

# Git

Cherry currently offers a set of fundamental Git features, with support coming in the future for more advanced ones, like conflict resolution tools, line by line staging, and more.
...
Cherry monitors your repository so that changes you make on the command line are instantly reflected.

### Configuration

You can configure how Cherry hard wraps commit messages with the `preferred-line-length` setting of the "Git Commit" language. The default is `72`, but it can be set to any number of characters `0` or more.
...
You can see all of the changes captured by Git in Cherry by opening the Project Diff ({#kb git::Diff}), accessible via the {#action git::Diff} action in the Command Palette or the Git Panel.
...
Fetch, push, or pull from your Git repository in Cherry via the buttons available on the Git Panel or via the Command Palette by looking at the respective actions: {#action git::Fetch}, {#action git::Push}, and {#action git::Pull}.

## Staging Workflow

Cherry has two primary staging workflows, using either the Project Diff or the panel directly.
...
Cherry offers two commit textareas:
...
As soon as you commit in Cherry, in the Git Panel, you'll see a bar right under the commit textarea, which will show the recently submitted commit.
In there, you can use the "Uncommit" button, which performs the `git reset HEADˆ--soft` command.

### Configuring Commit Line Length

By default, Cherry sets the commit line length to `72` but it can be configured in your local `settings.json` file.
...
Cherry provides a comprehensive stash picker accessible via {#action git::ViewStash}. From the stash picker, you can:
...
For faster workflows, Cherry provides direct actions to work with the most recent stash:
...
Cherry currently supports LLM-powered commit message generation.
...
> Note that you need to have an LLM provider configured for billing purposes, either via your own API keys or trialing/paying for Cherry's hosted AI models. Visit [the AI configuration page](./ai/configuration.md) to learn how to do so.
...
Cherry integrates with popular Git hosting services to ensure that Git commit hashes and references to Issues, Pull Requests, and Merge Requests become clickable links.

Cherry currently supports links to the hosted versions of
...
Cherry also has a Copy Permalink feature to create a permanent link to a code snippet on your Git hosting service.
...
When viewing files with changes, Cherry displays diff hunks that can be expanded or collapsed for detailed review:
...
If you would like to also use Cherry for your [git commit message editor](https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration#_core_editor) when committing from the command line you can use `zed --wait`:
