...
Cherry's Helix mode is an emulation layer that brings Helix-style keybindings and modal editing to Cherry. It builds upon Cherry's [Vim mode](./vim.md), so much of the core functionality is shared. Enabling `helix_mode` will also enable `vim_mode`.
