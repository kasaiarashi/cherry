...
Cherry has a very customizable key binding system—you can tweak everything to work exactly how your fingers expect!
...
In there, you can see all of the existing actions in Cherry as well as the associated keybindings set to them by default.
...
- Windows: `~\AppData\Roaming\Cherry/keymap.json`
...
You can see all of Cherry's default bindings for each platform in the default keymaps files:
...
Cherry has the ability to match against not just a single keypress, but a sequence of keys typed in order. Each key in the `"bindings"` map is a sequence of keypresses separated with a space.
...
- `shift-` The shift key
- `fn-` The function key
- `secondary-` Equivalent to `cmd` when Cherry is running on macOS and `ctrl` when on Windows and Linux
...
The `alt-` modifier can be used on many layouts to generate a different key. For example, on a macOS US keyboard, the combination `alt-c` types `ç`. You can match against either in your keymap file, though by convention, Cherry spells this combination as `alt-c`.
...
If a binding group has a `"context"` key, it will be matched against the currently active contexts in Cherry.
...
On Cyrillic, Hebrew, Armenian, and other keyboards that are mostly non-ASCII, macOS automatically maps keys to the ASCII range when `cmd` is held. Cherry takes this a step further, and it can always match key-presses against either the ASCII layout or the real layout, regardless of modifiers and the `use_key_equivalents` setting. For example, in Thai, pressing `ctrl-ๆ` will match bindings associated with `ctrl-q` or `ctrl-ๆ`.
...
This is useful for preventing Cherry from falling back to a default key binding when the action you specified is conditional and propagates. For example, `buffer_search::DeployReplace` only triggers when the search bar is not in view. If the search bar is in view, it would propagate and trigger the default action set for that key binding, such as opening the right dock. To prevent this from happening:
...
If you're on Linux or Windows, you might find yourself wanting to forward key combinations to the built-in terminal instead of them being handled by Cherry.

For example, `ctrl-n` creates a new tab in Cherry on Linux. If you want to send `ctrl-n` to the built-in terminal when it's focused, add the following to your keymap:
...
You can also bind keys to launch Cherry Tasks defined in your `tasks.json`.
