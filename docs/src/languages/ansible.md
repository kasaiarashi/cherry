...
Support for Ansible in Cherry is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/zed-ansible).
...
- Or configure the yaml language server settings to set this schema for all your inventory files, that match your inventory pattern, under your Cherry settings ([ref](https://kriaa.in/cherry/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your Cherry settings file. For example:

```json [settings]
"lsp": {
  // Note, the Cherry Ansible extension prefixes all settings with `ansible`
