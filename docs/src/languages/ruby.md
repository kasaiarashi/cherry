...
There are multiple language servers available for Ruby. Cherry supports the two following:
...
In addition to these two language servers, Cherry also supports:

- [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by Cherry as a language server, but its functionality is complimentary to that of solargraph and ruby-lsp.
...
Solargraph has formatting and diagnostics disabled by default. We can tell Cherry to enable them by adding the following to your `settings.json`:
...
Rubocop has unsafe autocorrection disabled by default. We can tell Cherry to enable it by adding the following to your `settings.json`:
...
The Ruby extension provides a debug adapter for debugging Ruby code. Cherry's name for the adapter (in the UI and `debug.json`) is `rdbg`, and under the hood, it uses the [`debug`](https://github.com/ruby/debug) gem. The extension uses the [same activation logic](#language-server-activation) as the language servers.
