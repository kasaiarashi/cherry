...
Cherry supports auto-formatting SQL using external tools like [`sql-formatter`](https://github.com/sql-formatter-org/sql-formatter).
...
3. Configure Cherry to automatically format SQL with `sql-formatter`:
...
You can add this to Cherry project settings (`.zed/settings.json`) or via your Cherry user settings (`~/.config/zed/settings.json`).
...
When using a `.sql-formatter.json` file you can use a more simplified set of Cherry settings since the language need not be specified inline:
