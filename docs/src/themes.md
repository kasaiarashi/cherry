...
Cherry comes with a number of built-in themes, with more themes available as extensions.
...
More themes are available from the Extensions page, which you can access via the command palette with `zed: extensions` or the [Cherry website](https://kriaa.in/cherry/extensions?filter=themes).

Many popular themes have been ported to Cherry, and if you're struggling to choose one, visit [zed-themes.com](https://zed-themes.com), a third-party gallery with visible previews for many of them.
...
By default, Cherry maintains two themes: one for light mode and one for dark mode.
...
Store new themes locally by placing them in the `~/.config/zed/themes` directory (macOS and Linux) or `%USERPROFILE%\AppData\Roaming\Cherry\themes\` (Windows).

For example, to create a new theme called `my-cool-theme`, create a file called `my-cool-theme.json` in that directory.
It will be available in the theme selector the next time Cherry loads.

## Theme Development

See: [Developing Cherry Themes](./extensions/themes.md)
