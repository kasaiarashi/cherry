...
Various aspects of Cherry's visual layout can be configured via either the settings window or the `settings.json` file, which you can access via {#action zed::OpenSettings} ({#kb zed::OpenSettings}) and {#action zed::OpenSettingsFile} ({#kb zed::OpenSettingsFile}) respectively.

See [Configuring Cherry](./configuring-zed.md) for additional information and other non-visual settings.
...
By default Cherry enable font ligatures which will visually combines certain adjacent characters.
...
```json [settings]
{
  // Force usage of Cherry build in path prompts (file and directory pickers)
  // instead of OS native pickers (false).
  "use_system_path_prompts": true,
  // Force usage of Cherry built in confirmation prompts ("Do you want to save?")
...
See [Cherry AI Documentation](./ai/overview.md) for additional non-visual AI settings.
