...
FBlueprintService::FBlueprintService(TSharedPtr<FCherryLinkServer> InServer)
	: Server(InServer)
{
}
...
void FBlueprintService::Initialize()
{
	...
	UE_LOG(LogCherryLink, Log, TEXT("BlueprintService: Initialized"));
}
...
TSharedPtr<FBlueprintInfo> FBlueprintService::GetBlueprintDetails(const FString& AssetPath)
{
	...
	if (!Blueprint)
	{
		UE_LOG(LogCherryLink, Warning, TEXT("BlueprintService: Failed to load Blueprint: %s"), *AssetPath);
		return nullptr;
	}
...
bool FBlueprintService::OpenBlueprintInEditor(const FString& AssetPath, const FString& FocusNode)
{
	...
	if (!Blueprint)
	{
		UE_LOG(LogCherryLink, Warning, TEXT("BlueprintService: Failed to load Blueprint for opening: %s"), *AssetPath);
		return false;
	}
	...
	if (GEditor)
	{
		...
		if (AssetEditorSubsystem)
		{
			...
			UE_LOG(LogCherryLink, Log, TEXT("BlueprintService: Opened Blueprint: %s"), *AssetPath);
			return true;
		}
	}
