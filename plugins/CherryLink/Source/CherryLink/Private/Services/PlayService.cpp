...
FPlayService::FPlayService(TSharedPtr<FCherryLinkServer> InServer)
	: Server(InServer)
{
}
...
void FPlayService::Initialize()
{
	...
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: Initialized"));
}
...
bool FPlayService::StartPIE(const FString& Mode, int32 PlayerCount, bool bDedicatedServer)
{
	if (!GUnrealEd)
	{
		UE_LOG(LogCherryLink, Error, TEXT("PlayService: GUnrealEd not available"));
		return false;
	}

	if (IsPlaying())
	{
		UE_LOG(LogCherryLink, Warning, TEXT("PlayService: PIE session already running"));
		return false;
	}
	...
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: Starting %s session"), *Mode);
...
bool FPlayService::StopPIE()
{
	...
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: Stopping PIE session"));
...
bool FPlayService::PausePIE()
{
	...
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: Paused PIE session"));
	return true;
}
...
bool FPlayService::ResumePIE()
{
	...
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: Resumed PIE session"));
	return true;
}
...
{
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: PIE started (simulating: %s)"), bSimulating ? TEXT("true") : TEXT("false"));
	BroadcastStateChange();
}

void FPlayService::OnPIEEnded(bool bSimulating)
{
	UE_LOG(LogCherryLink, Log, TEXT("PlayService: PIE ended"));
	BroadcastStateChange();
}
